...
export function AdminUserManagement() {
  ...
  const promoteToClubHead = async () => {
    ...
    try {
      ...
      setMessage(`✅ Promoted ${selectedUsers.size} user(s) to Club Head`);
      setTimeout(() => setMessage(""), 3000);
    } catch (error) {
      console.error("❌ Error promoting users:", error);
      setMessageType("error");
      setMessage("Failed to promote users");
      setTimeout(() => setMessage(""), 3000);
    }
  };
  ...
  const demoteToNormalUser = async () => {
    ...
    try {
      ...
      setSelectedUsers(new Set());
      setMessageType("success");
      setMessage(`✅ Demoted ${selectedUsers.size} user(s) to Normal User`);
      setTimeout(() => setMessage(""), 3000);
    } catch (error) {
      console.error("❌ Error demoting users:", error);
      setMessageType("error");
      setMessage("Failed to demote users");
      setTimeout(() => setMessage(""), 3000);
    }
  };
